...
{
    public class BattleUnitBuf_BleedCrystal : BattleUnitBuf
    {
        ...
        public static void GainBuf(BattleUnitModel model, int v)
        {
            ...
            {
                (model.bufListDetail.GetFieldValue<List<BattleUnitBuf>>("_bufList").Find(x => x is BattleUnitBuf_BleedCrystal) as BattleUnitBuf_BleedCrystal).stack += v;
            }
            else
            {
                var buf = new BattleUnitBuf_BleedCrystal();
                buf.stack = v;
